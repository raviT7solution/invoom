...
import { AUTH_TOKEN, TEST_API_URL } from "../../../utils/Constant";
...
const Edit = ({
  ...
  const onFinish = async (values: schema) => {
    try {
      setLoading(true);
      const url = isNew
        ? `${TEST_API_URL}/v1/features/create`
        : `${TEST_API_URL}/v1/features/edit/${menuId}`;
  ...
  const fetchFeatureById = async () => {
    ...
    try {
      const res = await fetch(`${TEST_API_URL}/v1/features/${menuId}`, {
        headers: {
          Authorization: AUTH_TOKEN,
        },
      });
